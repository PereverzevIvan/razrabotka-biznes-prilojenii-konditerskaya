...
func (service *productService) CalcComponentsMaxDeliveryTime(component_ids []int) (int, error) {
	var components_max_deliver_time int = 0

	for _, component_id := range component_ids {
		delivery_time_minutes, err := service.supplierComponentRepo.FastestDeliveryComponentTime(component_id)
		if err != nil {
			return 0, err
		}
