...
func (usecase *ProductUsecase) CalcProductionPrice(product_id int) (
	total_price float64,
	purchased_components_prices map[int]float64,
	to_purchase_components_prices map[int]float64,
	err error,
) {
	needed_recipe_components_map, err := usecase.CountNeededRecipeComponents(product_id)
	if err != nil {
	...
	for component_id, count_needed := range needed_recipe_components_map {
		...
		remaining_count, err := usecase.purchasedComponentRepo.CountRemainingComponents(component_id)
		if err != nil {
			return 0, nil, nil, err
		}
		...
		remaining_components_price, err := usecase.purchasedComponentRepo.
		...
		price_per_component, err := usecase.supplierComponentRepo.FastestDeliveryComponentPrice(component_id)
		if err != nil {
			switch {
