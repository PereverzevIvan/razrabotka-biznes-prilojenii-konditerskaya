...
func (c *productController) GetProductionInfo(ctx fiber.Ctx) error {
	product_id := fiber.Params[int](ctx, "product_id")
	if product_id <= 0 {
		return ctx.Status(fiber.StatusBadRequest).SendString("invalid product id")
	}

	count_needed_components, err := c.productUsecase.CountNeededRecipeComponents(product_id)
	if err != nil {
		log.Error(err)
		switch {
	...
	total_price,
		...
		err := c.productUsecase.CalcProductionPrice(product_id)
	if err != nil {
		log.Error(err)
		switch {
	...
	max_deliver_time, err := c.productUsecase.CalcComponentsMaxDeliveryTime(components_to_purchase_ids)
	if err != nil {
		log.Error(err)
		return ctx.SendStatus(fiber.StatusInternalServerError)
	}
